...
data_folder = '/content/drive/MyDrive/abide_dataset/ABIDE_pcp/ccs/filt_noglobal'
phenotype = '/content/drive/MyDrive/abide_dataset/data/Phenotypic_V1_0b_preprocessed1.csv'
...
def fetch_filenames_extract_quality_checked(subject_IDs, file_type, atlas):
    """
    Fetch filenames for the given subject IDs, file type, and atlas.
    Extract only quality checked subjects to a txt file
    I know, not the best design 

    Parameters:
        subject_IDs : list of short subject IDs in string format
        file_type   : must be one of the available file types
        atlas       : atlas name to use in file mapping

    Returns:
        filenames   : list of valid file paths (same length as subject_list)
    """
    ...
    for subject_id in subject_IDs:
        try:
            # Use glob to search for files containing the subject ID and file type pattern
            os.chdir(data_folder)
            search_pattern = f"*{subject_id}*{filemapping[file_type]}"
            matches = glob.glob(search_pattern)
            matches = glob.glob('*' + subject_id + filemapping[file_type])
            if matches:
                filenames.append(matches[0])  # Add the first match found
                valid_subject_IDs.append(subject_id)  # Save the valid subject ID
            else:
    ...
    # Save valid subject IDs to a text file
    with open("subject_IDs_quality_checked_regular_time.txt", "w") as f:
        f.write("\n".join(valid_subject_IDs))
...
def get_ids(num_subjects=None):
    ...
    subject_IDs = np.genfromtxt(os.path.join(data_folder, 'subject_IDs_quality_checked_regular_time.txt'), dtype=str)
...
def get_networks(subject_list, kind, iter_no='', seed=1234, n_subjects='', atlas_name="aal",
                 variable='connectivity', min_abs_value=0.0):
    """
    ...
    for subject in subject_list:
        ...
        if min_abs_value > 0:
            matrix[np.abs(matrix) < min_abs_value] = 0
